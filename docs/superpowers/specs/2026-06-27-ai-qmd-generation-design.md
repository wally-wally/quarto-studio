...
- **헬퍼**: `src/lib/ai/settings.ts`에 `loadSettings()`, `saveSettings()`, 활성 프로바이더의 `{ apiKey, model }`을 꺼내는 `getActiveCredentials()`를 둔다. 키가 비어 있으면 생성 시 "설정에서 API 키를 입력하세요" 인라인 안내와 설정 모달 링크를 노출한다(생성 버튼 자체는 활성 — 설정으로 가는 경로를 막지 않기 위함).
...
| 상황 | 동작 |
| --- | --- |
| API 키 미설정 | 생성 시 "설정에서 API 키를 입력하세요" 인라인 오류 + 설정 모달 링크(생성 버튼은 활성) |
