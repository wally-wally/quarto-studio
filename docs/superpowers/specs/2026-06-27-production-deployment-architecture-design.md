...
`jupyter: julia-1.10` 같은 front matter는 **머신마다 다른 커널 이름**에 문서를 묶는다. 로컬에 `julia-1.12`만 있으면 `kernel 'julia-1.10' not found`로 깨진다. altair 누락, `.venv` 활성화 깜빡과 같은 부류 — 렌더가 머신의 가변 상태에 의존하는 문제다. 해법도 동일하다: **이미지가 실행 환경의 단일 소스**가 된다.

- 이미지는 언어별로 **단일·핀 고정** 런타임과 **정해진 커널 이름**을 등록한다(예: Julia 1.x + 커널 `julia-1.10`). 렌더 환경이 하나뿐이므로 커널 이름이 항상 결정적으로 해소된다.
- 새 문서 템플릿(`createDefaultContent`)과 예제 `.qmd`는 이 **정규 이름만** 사용한다. 문서에 머신 종속 식별자를 넣지 않으며, 예제는 이미지의 정규 이름에 한 번 정렬한다.
- **Julia의 더 나은 목표**: Quarto 네이티브 Julia 엔진(`engine: julia`, QuartoNotebookRunner)을 쓰면 커널 이름 문자열 자체가 사라진다. 이미지에 러너를 한 번 설정하면 버전-이름 종속이 제거된다. Phase 0에서 "커널 이름 핀(단순)" vs "네이티브 엔진(견고, 러너 설정 필요)" 중 확정.
- Python도 같은 원리: 이미지의 venv·커널이 고정되어 `.venv` 활성화/`QUARTO_PYTHON` 문제(Python판 동일 이슈)가 사라진다.

> 예제를 `julia-1.12`로 바꾸는 것은 **이 머신용 임시 해소**일 뿐, 다른 머신·Julia 업그레이드에서 또 깨진다. 근본 해법은 이미지 핀이다.
...
- **Phase 0 — 렌더 이미지 + 폰트**: `quarto-render` 이미지 빌드(Quarto + 3런타임 + 한글 폰트 + 엔진 설정). 패키지는 버전 핀 매니페스트로 정의한 **큐레이션 baseline**(altair·vega_datasets 등 흔한 스택 포함). **실행 환경(커널·엔진) 핀** 및 새 문서 템플릿·예제의 정규 이름 정렬 포함. 예제 `.qmd`와 한글 차트로 독립 검증. **한글 폰트 · "패키지 누락(altair 등)" · "커널 버전 불일치(julia-1.10/1.12)" 통증을 함께 즉시 해소**하고 이후 모든 단계의 전제가 된다.
...
- Julia 실행: 커널 이름 핀(단순) vs 네이티브 `engine: julia`(견고, QuartoNotebookRunner 설정 필요) — Phase 0에서 확정.
