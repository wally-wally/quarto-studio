...
Debian-slim 기반에 다음을 굽는다.

- Quarto CLI
- Python venv: jupyter, matplotlib, pandas, numpy 등
- R: knitr, rmarkdown, ggplot2, showtext
- Julia: IJulia, Plots
- 한글 폰트(섹션 5) 및 엔진별 폰트 기본 설정

런타임 3종이 모두 들어가 수 GB 이미지가 된다. 레이어 분리와 빌드 캐시로 관리한다. 사용자는 어떤 런타임도 설치할 필요가 없어진다.
...
- **Phase 0 — 렌더 이미지 + 폰트**: `quarto-render` 이미지 빌드(Quarto + 3런타임 + 한글 폰트 + 엔진 설정). 예제 `.qmd`와 한글 차트로 독립 검증. **한글 폰트 통증을 즉시 해소**하고 이후 모든 단계의 전제가 된다.
